...
class UserSync {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.data.quizStats) {
          localStorage.setItem('quizStats', JSON.stringify(data.data.quizStats));
        }
  ...
  async syncToServer() {
    ...
    try {
      const quizStats = JSON.parse(localStorage.getItem('quizStats') || '{"quizzes":[], "totalQuizzes":0}');
  ...
  overrideLocalStorage() {
    ...
    localStorage.setItem = (key, value) => {
      ...
      // Sync relevant data to server
      if (key === 'quizStats' || key === 'darkMode') {
        // Debounce sync calls
...
window.addEventListener('beforeunload', () => {
  if (window.userSync && window.userSync.isAuthenticated) {
    ...
    navigator.sendBeacon('/auth/sync-data', data);
  }
});